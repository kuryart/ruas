...
class TableWidget extends WidgetType {
	...
	toDOM() {
		...
		const splitCells = (line: string) => splitTableCells(line);
	...
	syntaxTree(state).iterate({
		enter(node) {
			...
			if (name === 'Table') {
				...
				let endLine = state.doc.lineAt(Math.max(node.from, node.to - 1));
				// Expand downward for wiki-link pipes that confuse the parser
				while (endLine.number < state.doc.lines) {
					const nxt = state.doc.line(endLine.number + 1).text;
					if (nxt.includes('|') && splitTableCells(nxt).length >= 2) endLine = state.doc.line(endLine.number + 1);
					else break;
				}
				const src = state.sliceDoc(startLine.from, endLine.to).trim();
