 ...
 *
 * Positioning: the popup appears *below* the text cursor by default. When it
 * would overflow the viewport bottom it flips to appear *above* the cursor
 * instead. Horizontal overflow is clamped to keep the popup fully on-screen.
 */
export default function FuzzyPopup() {
	...
	// ── Positioning ──────────────────────────────────────────────────────────
	//
	// We re-measure after each item-list change so that the flip decision is
	// always based on the actual rendered height of the popup.

	createEffect(() => {
		const st = fuzzyState();
		const items = ranked();       // tracked dependency
		if (!st || !listRef) return;

		queueMicrotask(() => {
			if (!listRef) return;
			const popup = listRef.getBoundingClientRect();
			...
			// Vertical — prefer below; flip above when it overflows the bottom.
			if (anchor.y + gap + popupH <= vh - 8) {
				setPopupTop(anchor.y + gap);
			} else {
				setPopupTop(anchor.y - gap - gapTop - popupH);
			}
		});
	});
